function [] = cl_analyze_fun(model, ocp, dt, x_history, u_history, p_swarm, map, ...
    status, sqp_iter, time_tot, results_folder)

%% Create output folder

date_string = datestr(now,'yyyy_mm_dd_HH_MM_SS');
dirname = strcat(results_folder,date_string);
...
%% Plot swarm state variables (with SwarmLib)
...
[safety, order, union, alg_conn, safety_obs, min_d ] = ...
    compute_swarm_performance(pos_history, vel_history, ...
    p_swarm, dirname);

% Plot performance
plot_swarm_performance(time_history, safety, order, ...
    union, alg_conn, safety_obs, min_d, p_swarm, fontsize, dirname);


%% Show solver convergence

if (strcmp(ocp.opts_struct.nlp_solver, 'sqp'))
    figure;
...
if (strcmp(ocp.opts_struct.nlp_solver, 'sqp_rti'))
