function [model, ocp, sim, map, p_swarm, p, nb_steps_sim, x0] = cl_init_fun(...
    dt, T, model, ocp, sim, map, p_swarm)

% cl_init - closed-loop mpc initialization
% 
% Inputs:
%   dt - sample time [s]
%   nb_obstacles - number of obstacles per row in the map
%   d_ref
% 
% Outputs: 
%   
...
%   map - map params structure
%   S - swarm params structure
%   p - prediction window steps
%   nb_steps_sim - number of steps of the simulation
%   x0 - swarm problem initialization (initial state)
%
...
nb_steps_sim = floor(T/dt); % maximum nb of steps over the whole simulation
