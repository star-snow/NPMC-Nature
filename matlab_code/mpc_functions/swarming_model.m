function model = swarming_model(p_swarm)
...
for agent = 1:N
    ...
    for j = 1:(N-1)
        ...
        sym_sep((agent-1)*(N-1)+j) = pos_rel'*pos_rel - d_ref^2;
    end
...
% Weights
W = [W_sep/(N-1)*ones(N*(N-1),1); W_dir*ones(N,1); ...
    W_nav*ones(N,1); W_u*W_sep*ones(3*N,1)];
W_e = [W_sep*ones(N*(N-1),1); W_dir*ones(N,1); ...
    W_nav*ones(N,1)];

% Assemble expr_y
expr_y = vertcat(sym_sep, sym_dir, sym_nav, sym_u);
